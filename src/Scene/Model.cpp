...
void Model::Draw(Shader& shader,int instanceAmount)
{
	for (unsigned int i = 0; i < (unsigned int)meshes.size(); i++) {
...
Mesh::Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	return Mesh::Mesh(vertices, indices, textures, mat);


}

std::vector<Mesh::Texture> Model::loadMaterialTextures(aiMaterial* materials, aiTextureType type, std::string typeName)
{
	std::vector<Mesh::Texture> textures;
	for (unsigned int i = 0; i < materials->GetTextureCount(type); i++)
	{
		aiString str;
		...
		{   // if texture hasn't been loaded already, load it
			...
			TextureLoaded.push_back(texture);  // store it as texture loaded for entire model, to ensure we won't unnecessary load duplicate textures.
		}
	}
	return textures;
}
